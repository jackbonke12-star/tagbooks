...
export const ITEMS = [
  ...
  { value: 'stickers', label: 'NFC Tags', reorderUrl: 'https://www.amazon.ca/dp/B0GGN9H41G' },
  { value: 'filament_rolls', label: 'Filament Rolls', reorderUrl: 'https://www.amazon.ca/s?k=PLA+filament+1.75mm' },
];

// What one unit of each product consumes from inventory_materials. Keys are
// PRODUCTS `value`s; each line is { kind, qty } where kind is a material kind
// (review_sticker | nfc_tag | adhesive | filament | other) and qty is in that
// material's unit (pcs for stickers/tags, ml for adhesive, GRAMS for filament).
// consumeForSale() multiplies these by the sale quantity. Products not listed
// here consume nothing. Assumptions (edit freely as real usage is measured):
//   - Kits ship a 3D-printed part + a blank NFC tag: ~8g filament + 1 nfc_tag.
//     The custom logo kit prints a larger logo stand, so ~14g filament.
//   - NFC stickers / custom stickers apply one Google review sticker each.
//   - logo_stands are a 3D-printed part: ~12g filament, no tag.
//   - Software/website/menu/care products consume no physical stock.
export const CONSUMPTION = {
  ...
  custom_logo_kit: [
    { kind: 'filament', qty: 14 },
    { kind: 'nfc_tag', qty: 1 },
    { kind: 'review_sticker', qty: 1 },
  ],
  logo_stands: [{ kind: 'filament', qty: 12 }],
  // Present so future sticker products decrement review stickers out of the box.
  nfc_sticker: [{ kind: 'review_sticker', qty: 1 }],
  custom_sticker: [{ kind: 'review_sticker', qty: 1 }],
};
...
// The single rule for converting a Places prospect into a client: a prospect
// that's already been pitched/won becomes a 'pitched' client; anything earlier
// in the funnel starts as a 'lead'. Used by BOTH conversion entry points
// (Places "Add as client" and Clients "Pull from Places") so they never diverge.
export function stageForProspect(status) {
  return status === 'pitched' || status === 'won' ? 'pitched' : 'lead';
}
